...
while True:
    separou_ingredientes = input("Você separou todos os ingredientes necessários? (s/n): ").strip().lower()
...
print("Untar a forma.")
while True:
    untar_forma = input("Você já untou a forma? (s/n): ").strip().lower()
    if untar_forma == "s":
...
while True:
    massa_pronta = input("Você já despejou a massa na forma? (s/n): ").strip().lower()
    if massa_pronta == "s":
...
while True:
    forno_pronto = input("Você já pré-aqueceu o forno? (s/n): ").strip().lower()
    if forno_pronto == "s":
...
//#Etapa 7: Preparar a Calda
print("Agora, vamos preparar a calda!")
while True:
    ingredientes_calda = input("Você separou os ingredientes para a calda? (s/n): ").strip().lower()
...
print("Agora, vamos preparar a calda.")
while True:
    calda_no_fogo = input("Você colocou a calda no fogo? (s/n): ").strip().lower()
    if calda_no_fogo == "s":
...
while True:
    calda_esfriando = input("A calda já esfriou? (s/n): ").strip().lower()
...
while True:
    bolo_coberto = input("Você já cobriu o bolo com a calda? (s/n): ").strip().lower()
    if bolo_coberto == "s":
